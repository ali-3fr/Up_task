...
public class ServerHandler implements HttpHandler {
    ...
    private Response doPut(HttpExchange httpExchange) {
        try {
            Message message = MessageHelper.putMessage(httpExchange.getRequestBody());
            logger.info(String.format("Edited message with following id %s", message.getId()));
    ...
    private Response doDelete(HttpExchange httpExchange) {
        try {
            Message message = MessageHelper.delMessage(httpExchange.getRequestBody());
            logger.info(String.format("Deleted message with following id: %s", message.getId()));
            messageStorage.removeMessage(message);
            return Response.ok();
        } catch (ParseException e) {
            logger.error("Could not parse message.", e);
            return new Response(Constants.RESPONSE_CODE_BAD_REQUEST, "Incorrect request body");
